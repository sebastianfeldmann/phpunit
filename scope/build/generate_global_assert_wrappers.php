...
';
$usedClasses = [];
$class = new \ReflectionClass(\PHPUnit\Framework\Assert::class);
$constraintMethods = '';
foreach ($class->getMethods() as $method) {
    ...
    if (!$returnType->isSubclassOf(\PHPUnit\Framework\Constraint\Constraint::class)) {
        continue;
    }
    $usedClasses[] = $returnType->getName();
    $constraintMethods .= \sprintf("%s\n{\n    return Assert::%s(...\\func_get_args());\n}\n\n", \str_replace('public static ', '', \trim($lines[$method->getStartLine() - 1])), $method->getName());
}
$usedClasses = \array_unique($usedClasses);
\sort($usedClasses);
foreach ($usedClasses as $usedClass) {
    $buffer .= \sprintf("use %s;\n", $usedClass);
}
$buffer .= "\n";
foreach ($class->getMethods() as $method) {
    ...
    $buffer .= \sprintf("%s\n%s\n{\n    Assert::%s(...\\func_get_args());\n}\n\n", \str_replace('     *', ' *', $method->getDocComment()), \str_replace('public static ', '', \trim($lines[$method->getStartLine() - 1])), $method->getName());
}
