...
class Command
{
    ...
    {
        try {
            $this->options = \PHPUnit\Util\Getopt::getopt($argv, 'd:c:hv', \array_keys($this->longOptions));
        ...
        if (isset($this->arguments['test']) && \is_string($this->arguments['test']) && \substr($this->arguments['test'], -5, 5) == '.phpt') {
            $test = new \PHPUnit\Runner\PhptTestCase($this->arguments['test']);
...
EOT;
    ...
    {
        $facade = new \_PhpScoper5bf3cbdac76b4\SebastianBergmann\FileIterator\Facade();
        foreach ($facade->getFilesAsArray($directory, '.phar') as $file) {
            ...
            try {
                $applicationName = new \PharIo\Manifest\ApplicationName('phpunit/phpunit');
                $version = new \PharIo\Version\Version(\PHPUnit\Runner\Version::series());
                $manifest = \PharIo\Manifest\ManifestLoader::fromFile('phar://' . $file . '/manifest.xml');
