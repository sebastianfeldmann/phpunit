...
use _PhpScoper5bf3cbdac76b4\SebastianBergmann\Environment\Console;
...
class ResultPrinter extends \PHPUnit\Util\Printer implements \PHPUnit\Framework\TestListener
{
    ...
    public function __construct($out = null, bool $verbose = \false, $colors = self::COLOR_DEFAULT, bool $debug = \false, $numberOfColumns = 80, bool $reverse = \false)
    {
        ...
        if (!\in_array($colors, self::AVAILABLE_COLORS, \true)) {
            throw \PHPUnit\Util\InvalidArgumentHelper::factory(3, \vsprintf('value from "%s", "%s" or "%s"', self::AVAILABLE_COLORS));
        }
        if (!\is_int($numberOfColumns) && $numberOfColumns !== 'max') {
            throw \PHPUnit\Util\InvalidArgumentHelper::factory(5, 'integer or "max"');
        }
        $console = new \_PhpScoper5bf3cbdac76b4\SebastianBergmann\Environment\Console();
