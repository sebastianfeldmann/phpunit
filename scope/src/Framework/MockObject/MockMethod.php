...
final class MockMethod
{
    ...
    public static function fromReflection(\ReflectionMethod $method, bool $callOriginalMethod, bool $cloneArguments) : self
    {
        ...
        return new self($method->getDeclaringClass()->getName(), $method->getName(), $cloneArguments, $modifier, self::getMethodParameters($method), self::getMethodParameters($method, \true), $returnType, $reference, $callOriginalMethod, $method->isStatic(), $deprecation, $method->hasReturnType() && $method->getReturnType()->allowsNull());
    }
    ...
    public function generateCode() : string
    {
        ...
        if ($returnType === 'parent') {
            ...
            if ($parentClass === \false) {
                throw new \PHPUnit\Framework\MockObject\RuntimeException(\sprintf('Cannot mock %s::%s because "parent" return type declaration is used but %s does not have a parent class', $this->className, $this->methodName, $this->className));
            }
    ...
    private function getTemplate(string $template) : \_PhpScoper5bf3cbdac76b4\Text_Template
    {
        $filename = __DIR__ . \DIRECTORY_SEPARATOR . 'Generator' . \DIRECTORY_SEPARATOR . $template;
        if (!isset(self::$templates[$filename])) {
            self::$templates[$filename] = new \_PhpScoper5bf3cbdac76b4\Text_Template($filename);
        }
            ...
            if (!$forCall) {
                ...
                if ($parameter->hasType() && (string) $parameter->getType() !== 'self') {
                    ...
                    try {
                        ...
                        throw new \PHPUnit\Framework\MockObject\RuntimeException(\sprintf('Cannot mock %s::%s() because a class or ' . 'interface used in the signature is not loaded', $method->getDeclaringClass()->getName(), $method->getName()), 0, $e);
                    }
                    if ($class !== null) {
