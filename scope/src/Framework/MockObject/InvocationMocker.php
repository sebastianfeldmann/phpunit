 ...
 */
class InvocationMocker implements \PHPUnit\Framework\MockObject\Stub\MatcherCollection, \PHPUnit\Framework\MockObject\Invokable, \PHPUnit\Framework\MockObject\Builder\NamespaceMatch
{
    /**
    ...
    {
        if (isset($this->builderMap[$id])) {
            throw new \PHPUnit\Framework\MockObject\RuntimeException('Match builder with id <' . $id . '> is already registered.');
        }
    ...
    public function invoke(\PHPUnit\Framework\MockObject\Invocation $invocation)
    {
        ...
        if ($this->returnValueGeneration === \false) {
            ...
            if (\strtolower($invocation->getMethodName()) === '__tostring') {
                $this->addMatcher(new \PHPUnit\Framework\MockObject\Matcher\DeferredError($exception));
                return '';
            }
            throw $exception;
        }
