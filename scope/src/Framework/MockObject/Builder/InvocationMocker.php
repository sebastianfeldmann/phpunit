...
class InvocationMocker implements \PHPUnit\Framework\MockObject\Builder\MethodNameMatch
{
    ...
    public function __construct(\PHPUnit\Framework\MockObject\Stub\MatcherCollection $collection, \PHPUnit\Framework\MockObject\Matcher\Invocation $invocationMatcher, array $configurableMethods)
    {
        ...
        $this->matcher = new \PHPUnit\Framework\MockObject\Matcher($invocationMatcher);
    ...
    public function willReturn($value, ...$nextValues)
    {
        if (\count($nextValues) === 0) {
            $stub = new \PHPUnit\Framework\MockObject\Stub\ReturnStub($value);
        } else {
            $stub = new \PHPUnit\Framework\MockObject\Stub\ConsecutiveCalls(\array_merge([$value], $nextValues));
        }
    ...
    public function with(...$arguments)
    {
        ...
        $this->matcher->setParametersMatcher(new \PHPUnit\Framework\MockObject\Matcher\Parameters($arguments));
        return $this;
    }
    /**
     * @param array ...$arguments
    ...
    {
        $this->canDefineParameters();
        $this->matcher->setParametersMatcher(new \PHPUnit\Framework\MockObject\Matcher\ConsecutiveParameters($arguments));
        return $this;
    }
    /**
     * @throws RuntimeException
    ...
    {
        $this->canDefineParameters();
        $this->matcher->setParametersMatcher(new \PHPUnit\Framework\MockObject\Matcher\AnyParameters());
        return $this;
    }
    /**
    ...
    public function method($constraint)
    {
        if ($this->matcher->hasMethodNameMatcher()) {
            throw new \PHPUnit\Framework\MockObject\RuntimeException('Method name matcher is already defined, cannot redefine');
        }
        ...
        $this->matcher->setMethodNameMatcher(new \PHPUnit\Framework\MockObject\Matcher\MethodName($constraint));
        return $this;
    }
    /**
    ...
    {
        if (!$this->matcher->hasMethodNameMatcher()) {
            throw new \PHPUnit\Framework\MockObject\RuntimeException('Method name matcher is not defined, cannot define parameter ' . 'matcher without one');
        }
        if ($this->matcher->hasParametersMatcher()) {
            throw new \PHPUnit\Framework\MockObject\RuntimeException('Parameter matcher is already defined, cannot redefine');
        }
    }
}
