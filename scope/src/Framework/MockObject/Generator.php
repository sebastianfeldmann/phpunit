...
use _PhpScoper5bf3cbdac76b4\Doctrine\Instantiator\Exception\ExceptionInterface as InstantiatorException;
...
use _PhpScoper5bf3cbdac76b4\SoapClient;
...
class Generator
{
    ...
    public function getMock($type, $methods = [], array $arguments = [], $mockClassName = '', $callOriginalConstructor = \true, $callOriginalClone = \true, $callAutoload = \true, $cloneArguments = \true, $callOriginalMethods = \false, $proxyTarget = null, $allowMockingUnknownTypes = \true, $returnValueGeneration = \true)
    {
        if (!\is_array($type) && !\is_string($type)) {
            throw \PHPUnit\Util\InvalidArgumentHelper::factory(1, 'array or string');
        }
        if (!\is_string($mockClassName)) {
            throw \PHPUnit\Util\InvalidArgumentHelper::factory(4, 'string');
        }
        if (!\is_array($methods) && null !== $methods) {
            throw \PHPUnit\Util\InvalidArgumentHelper::factory(2, 'array', $methods);
        }
        ...
        if ($mockClassName !== '' && \class_exists($mockClassName, \false)) {
            ...
            if (!$reflect->implementsInterface(\PHPUnit\Framework\MockObject\MockObject::class)) {
    ...
    public function getMockForAbstractClass($originalClassName, array $arguments = [], $mockClassName = '', $callOriginalConstructor = \true, $callOriginalClone = \true, $callAutoload = \true, $mockedMethods = [], $cloneArguments = \true)
    {
        ...
        if (!\is_string($mockClassName)) {
            throw \PHPUnit\Util\InvalidArgumentHelper::factory(3, 'string');
        }
    ...
    public function generateClassFromWsdl($wsdlFile, $className, array $methods = [], array $options = [])
    {
        ...
        $client = new \_PhpScoper5bf3cbdac76b4\SoapClient($wsdlFile, $options);
    ...
    private function getObject($code, $className, $type = '', $callOriginalConstructor = \false, $callAutoload = \false, array $arguments = [], $callOriginalMethods = \false, $proxyTarget = null, $returnValueGeneration = \true)
    {
        ...
        if ($callOriginalConstructor && \is_string($type) && !\interface_exists($type, $callAutoload)) {
            ...
            try {
                $instantiator = new \_PhpScoper5bf3cbdac76b4\Doctrine\Instantiator\Instantiator();
                $object = $instantiator->instantiate($className);
            } catch (\_PhpScoper5bf3cbdac76b4\Doctrine\Instantiator\Exception\ExceptionInterface $exception) {
    ...
    private function generateMock($type, $explicitMethods, $mockClassName, $callOriginalClone, $callAutoload, $cloneArguments, $callOriginalMethods)
    {
        ...
        if (\is_array($explicitMethods) && empty($explicitMethods) && ($isClass || $isInterface)) {
