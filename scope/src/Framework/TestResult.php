...
use _PhpScoper5bf3cbdac76b4\SebastianBergmann\Invoker\TimeoutException;
use _PhpScoper5bf3cbdac76b4\SebastianBergmann\ResourceOperations\ResourceOperations;
...
class TestResult implements \Countable
{
    ...
    public function run(\PHPUnit\Framework\Test $test) : void
    {
        ...
        if ($this->convertErrorsToExceptions) {
            $oldErrorHandler = \set_error_handler([\PHPUnit\Util\ErrorHandler::class, 'handleError'], \E_ALL | \E_STRICT);
        ...
        $monitorFunctions = $this->beStrictAboutResourceUsageDuringSmallTests && !$test instanceof \PHPUnit\Framework\WarningTestCase && $test->getSize() == \PHPUnit\Util\Test::SMALL && \function_exists('_PhpScoper5bf3cbdac76b4\\xdebug_start_function_monitor');
        if ($monitorFunctions) {
            /* @noinspection ForgottenDebugOutputInspection */
            \_PhpScoper5bf3cbdac76b4\xdebug_start_function_monitor(\_PhpScoper5bf3cbdac76b4\SebastianBergmann\ResourceOperations\ResourceOperations::getFunctions());
        }
        \_PhpScoper5bf3cbdac76b4\SebastianBergmann\Timer\Timer::start();
        try {
            if (!$test instanceof \PHPUnit\Framework\WarningTestCase && $this->enforceTimeLimit && ($this->defaultTimeLimit || $test->getSize() != \PHPUnit\Util\Test::UNKNOWN) && \extension_loaded('pcntl') && \class_exists(\_PhpScoper5bf3cbdac76b4\SebastianBergmann\Invoker\Invoker::class)) {
                switch ($test->getSize()) {
                ...
                $invoker = new \_PhpScoper5bf3cbdac76b4\SebastianBergmann\Invoker\Invoker();
                $invoker->invoke([$test, 'runBare'], [], $_timeout);
            } else {
                $test->runBare();
            }
        } catch (\_PhpScoper5bf3cbdac76b4\SebastianBergmann\Invoker\TimeoutException $e) {
        ...
        $time = \_PhpScoper5bf3cbdac76b4\SebastianBergmann\Timer\Timer::stop();
        $test->addToAssertionCount(\PHPUnit\Framework\Assert::getCount());
        if ($monitorFunctions) {
            $blacklist = new \PHPUnit\Util\Blacklist();
            /** @noinspection ForgottenDebugOutputInspection */
            $functions = \_PhpScoper5bf3cbdac76b4\xdebug_get_monitored_functions();
            /* @noinspection ForgottenDebugOutputInspection */
            \_PhpScoper5bf3cbdac76b4\xdebug_stop_function_monitor();
