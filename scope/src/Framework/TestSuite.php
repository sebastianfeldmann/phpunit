...
class TestSuite implements \PHPUnit\Framework\Test, \PHPUnit\Framework\SelfDescribing, \IteratorAggregate
{
    ...
    public static function createTest(\ReflectionClass $theClass, $name) : \PHPUnit\Framework\Test
    {
        ...
        if (\count($parameters) < 2) {
            ...
            if (isset($data)) {
                $test = new \PHPUnit\Framework\DataProviderTestSuite($className . '::' . $name);
                if (empty($data)) {
    ...
    public function __construct($theClass = '', $name = '')
    {
        ...
        if (!$theClass->isSubclassOf(\PHPUnit\Framework\TestCase::class)) {
            throw new \PHPUnit\Framework\Exception('Class "' . $theClass->name . '" does not extend PHPUnit\\Framework\\TestCase.');
        }
        if ($name != '') {
            $this->setName($name);
        } else {
    ...
    public function addTestFiles($fileNames) : void
    {
        if (!(\is_array($fileNames) || \is_object($fileNames) && $fileNames instanceof \Iterator)) {
            throw \PHPUnit\Util\InvalidArgumentHelper::factory(1, 'array or iterator');
        }
