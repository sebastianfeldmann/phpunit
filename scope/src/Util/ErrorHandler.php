...
final class ErrorHandler
{
    ...
    public static function handleError(int $errorNumber, string $errorString, string $errorFile, int $errorLine) : bool
    {
        ...
        if ($errorNumber === \E_NOTICE || $errorNumber === \E_USER_NOTICE || $errorNumber === \E_STRICT) {
            if (\PHPUnit\Framework\Error\Notice::$enabled !== \true) {
                return \false;
            }
            $exception = \PHPUnit\Framework\Error\Notice::class;
        } elseif ($errorNumber === \E_WARNING || $errorNumber === \E_USER_WARNING) {
            if (\PHPUnit\Framework\Error\Warning::$enabled !== \true) {
                return \false;
            }
            $exception = \PHPUnit\Framework\Error\Warning::class;
        } elseif ($errorNumber === \E_DEPRECATED || $errorNumber === \E_USER_DEPRECATED) {
            if (\PHPUnit\Framework\Error\Deprecated::$enabled !== \true) {
                return \false;
            }
            $exception = \PHPUnit\Framework\Error\Deprecated::class;
        } else {
            $exception = \PHPUnit\Framework\Error\Error::class;
        }
