...
use _PhpScoper5bf3cbdac76b4\SebastianBergmann\Environment\OperatingSystem;
use Traversable;
final class Test
{
    /**
    ...
    public static function getRequirements(string $className, string $methodName) : array
    {
        ...
        if ($count = \preg_match_all(self::REGEX_REQUIRES_VERSION_CONSTRAINT, $docComment, $matches)) {
            foreach (\range(0, $count - 1) as $i) {
                ...
                try {
                    $versionConstraintParser = new \PharIo\Version\VersionConstraintParser();
    ...
    public static function getMissingRequirements(string $className, string $methodName) : array
    {
        ...
        if (!empty($required['PHPUnit'])) {
            ...
            $phpunitVersion = new \PharIo\Version\Version(self::sanitizeVersionNumber(\PHPUnit\Runner\Version::id()));
        ...
        if (!empty($required['OSFAMILY']) && $required['OSFAMILY'] !== (new \_PhpScoper5bf3cbdac76b4\SebastianBergmann\Environment\OperatingSystem())->getFamily()) {
    ...
    public static function getProvidedData(string $className, string $methodName) : ?array
    {
        ...
        if ($data !== null) {
            foreach ($data as $key => $value) {
                if (!\is_array($value)) {
                    throw new \PHPUnit\Framework\Exception(\sprintf('Data set %s is invalid.', \is_int($key) ? '#' . $key : '"' . $key . '"'));
                }
            }
        }
        return $data;
        ...
        if (\preg_match(self::REGEX_TEST_WITH, $docComment, $matches, \PREG_OFFSET_CAPTURE)) {
            ...
            foreach (\explode("\n", $annotationContent) as $candidateRow) {
                ...
                if (\json_last_error() !== \JSON_ERROR_NONE) {
                    throw new \PHPUnit\Framework\Exception('The data set for the @testWith annotation cannot be parsed: ' . \json_last_error_msg());
                }
                $data[] = $dataSet;
            }
            if (!$data) {
                throw new \PHPUnit\Framework\Exception('The data set for the @testWith annotation cannot be parsed.');
            }
            return $data;
        }
        return null;
