...
class ManifestDocument
{
    ...
    public static function fromFile($filename)
    {
        if (!\file_exists($filename)) {
            throw new \PharIo\Manifest\ManifestDocumentException(\sprintf('File "%s" not found', $filename));
        }
    ...
    public static function fromString($xmlString)
    {
        ...
        if (\count($errors) !== 0) {
            throw new \PharIo\Manifest\ManifestDocumentLoadingException($errors);
        }
        return new self($dom);
    }
    public function getContainsElement()
    {
        return new \PharIo\Manifest\ContainsElement($this->fetchElementByName('contains'));
    }
    public function getCopyrightElement()
    {
        return new \PharIo\Manifest\CopyrightElement($this->fetchElementByName('copyright'));
    }
    public function getRequiresElement()
    {
        return new \PharIo\Manifest\RequiresElement($this->fetchElementByName('requires'));
    }
    ...
    {
        return new \PharIo\Manifest\BundlesElement($this->fetchElementByName('bundles'));
    }
    private function ensureCorrectDocumentType(\DOMDocument $dom)
    {
        $root = $dom->documentElement;
        if ($root->localName !== 'phar' || $root->namespaceURI !== self::XMLNS) {
            throw new \PharIo\Manifest\ManifestDocumentException('Not a phar.io manifest document');
        }
    }
    /**
     * @param $elementName
    ...
    private function fetchElementByName($elementName)
    {
        ...
        if (!$element instanceof \DOMElement) {
            throw new \PharIo\Manifest\ManifestDocumentException(\sprintf('Element %s missing', $elementName));
        }
        return $element;
    }
}
