...
class ManifestSerializer
{
     ...
     */
    private $xmlWriter;
    public function serializeToFile(\PharIo\Manifest\Manifest $manifest, $filename)
    {
        \file_put_contents($filename, $this->serializeToString($manifest));
    }
    public function serializeToString(\PharIo\Manifest\Manifest $manifest)
    {
        $this->startDocument();
    ...
    private function addContains($name, \PharIo\Version\Version $version, \PharIo\Manifest\Type $type)
    {
    ...
    private function addCopyright(\PharIo\Manifest\CopyrightInformation $copyrightInformation)
    {
    ...
    private function addRequirements(\PharIo\Manifest\RequirementCollection $requirementCollection)
    {
        $phpRequirement = new \PharIo\Version\AnyVersionConstraint();
        $extensions = [];
        foreach ($requirementCollection as $requirement) {
            if ($requirement instanceof \PharIo\Manifest\PhpVersionRequirement) {
                $phpRequirement = $requirement->getVersionConstraint();
                continue;
            }
            if ($requirement instanceof \PharIo\Manifest\PhpExtensionRequirement) {
    ...
    private function addBundles(\PharIo\Manifest\BundledComponentCollection $bundledComponentCollection)
    {
    ...
    private function addExtension($application, \PharIo\Version\VersionConstraint $versionConstraint)
    {
