...
class ManifestDocumentMapperTest extends \PHPUnit\Framework\TestCase
{
    ...
    {
        $manifestDocument = \PharIo\Manifest\ManifestDocument::fromFile($expected);
        $mapper = new \PharIo\Manifest\ManifestDocumentMapper();
        $this->assertInstanceOf(\PharIo\Manifest\Manifest::class, $mapper->map($manifestDocument));
    }
    ...
    {
        $manifestDocument = \PharIo\Manifest\ManifestDocument::fromFile(__DIR__ . '/_fixture/custom.xml');
        $mapper = new \PharIo\Manifest\ManifestDocumentMapper();
        $this->expectException(\PharIo\Manifest\ManifestDocumentMapperException::class);
        $mapper->map($manifestDocument);
    }
    public function testInvalidVersionInformationThrowsException()
    {
        $manifestDocument = \PharIo\Manifest\ManifestDocument::fromFile(__DIR__ . '/_fixture/invalidversion.xml');
        $mapper = new \PharIo\Manifest\ManifestDocumentMapper();
        $this->expectException(\PharIo\Manifest\ManifestDocumentMapperException::class);
        $mapper->map($manifestDocument);
    }
    ...
    {
        $manifestDocument = \PharIo\Manifest\ManifestDocument::fromFile(__DIR__ . '/_fixture/invalidversionconstraint.xml');
        $mapper = new \PharIo\Manifest\ManifestDocumentMapper();
        $this->expectException(\PharIo\Manifest\ManifestDocumentMapperException::class);
        $mapper->map($manifestDocument);
    }
    ...
    {
        $manifestDocument = \PharIo\Manifest\ManifestDocument::fromFile(__DIR__ . '/_fixture/extension-invalidcompatible.xml');
        $mapper = new \PharIo\Manifest\ManifestDocumentMapper();
        $this->expectException(\PharIo\Manifest\ManifestDocumentMapperException::class);
        $mapper->map($manifestDocument);
    }
