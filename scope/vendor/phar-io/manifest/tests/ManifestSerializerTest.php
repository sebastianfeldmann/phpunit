...
class ManifestSerializerTest extends \PHPUnit\Framework\TestCase
{
    ...
    {
        $manifest = \PharIo\Manifest\ManifestLoader::fromString($expected);
        $serializer = new \PharIo\Manifest\ManifestSerializer();
    ...
    public function testCanSerializeToFile()
    {
        ...
        $dest = '/tmp/' . \uniqid('serializer', \true);
        $manifest = \PharIo\Manifest\ManifestLoader::fromFile($src);
        $serializer = new \PharIo\Manifest\ManifestSerializer();
    ...
    {
        $type = $this->getMockForAbstractClass(\PharIo\Manifest\Type::class);
        $manifest = new \PharIo\Manifest\Manifest(new \PharIo\Manifest\ApplicationName('testvendor/testname'), new \PharIo\Version\Version('1.0.0'), $type, new \PharIo\Manifest\CopyrightInformation(new \PharIo\Manifest\AuthorCollection(), new \PharIo\Manifest\License('bsd-3', new \PharIo\Manifest\Url('https://some/uri'))), new \PharIo\Manifest\RequirementCollection(), new \PharIo\Manifest\BundledComponentCollection());
        $serializer = new \PharIo\Manifest\ManifestSerializer();
