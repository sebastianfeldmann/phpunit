...
class PHP_Token_InterfaceTest extends \PHPUnit\Framework\TestCase
{
    /**
     * @var PHP_Token_CLASS
     */
    ...
    {
        $ts = new \_PhpScoper5bf3cbdac76b4\PHP_Token_Stream(\TEST_FILES_PATH . 'source4.php');
        $i = 0;
        foreach ($ts as $token) {
            if ($token instanceof \_PhpScoper5bf3cbdac76b4\PHP_Token_CLASS) {
                $this->class = $token;
            } elseif ($token instanceof \_PhpScoper5bf3cbdac76b4\PHP_Token_INTERFACE) {
    ...
    public function testGetPackageNamespace()
    {
        ...
        foreach ($tokenStream as $token) {
            if ($token instanceof \_PhpScoper5bf3cbdac76b4\PHP_Token_INTERFACE) {
                $package = $token->getPackage();
                $this->assertSame('_PhpScoper5bf3cbdac76b4\\Foo\\Bar', $package['namespace']);
            }
        }
    }
    ...
    {
        $tokenStream = new \_PhpScoper5bf3cbdac76b4\PHP_Token_Stream($filepath);
        $firstClassFound = \false;
        foreach ($tokenStream as $token) {
            if ($firstClassFound === \false && $token instanceof \_PhpScoper5bf3cbdac76b4\PHP_Token_INTERFACE) {
                $package = $token->getPackage();
            ...
            if ($token instanceof \_PhpScoper5bf3cbdac76b4\PHP_Token_INTERFACE) {
                ...
                $this->assertSame('_PhpScoper5bf3cbdac76b4\\Foo\\Baz', $package['namespace']);
                return;
            }
        }
    ...
    {
        $tokenStream = new \_PhpScoper5bf3cbdac76b4\PHP_Token_Stream(\TEST_FILES_PATH . 'classExtendsNamespacedClass.php');
        $firstClassFound = \false;
        foreach ($tokenStream as $token) {
            if ($firstClassFound === \false && $token instanceof \_PhpScoper5bf3cbdac76b4\PHP_Token_INTERFACE) {
                $package = $token->getPackage();
            ...
            if ($token instanceof \_PhpScoper5bf3cbdac76b4\PHP_Token_INTERFACE) {
                ...
                $this->assertSame('_PhpScoper5bf3cbdac76b4\\Other\\Space', $package['namespace']);
                return;
            }
        }
        $this->fail('Searching for 2 classes failed');
...
\class_alias('_PhpScoper5bf3cbdac76b4\\PHP_Token_InterfaceTest', 'PHP_Token_InterfaceTest', \false);
