...
class PHP_Token_FunctionTest extends \PHPUnit\Framework\TestCase
{
    /**
    ...
    {
        $ts = new \_PhpScoper5bf3cbdac76b4\PHP_Token_Stream(\TEST_FILES_PATH . 'source.php');
        foreach ($ts as $token) {
    ...
    {
        $ts = new \_PhpScoper5bf3cbdac76b4\PHP_Token_Stream(\TEST_FILES_PATH . 'source5.php');
        $f = $ts->getFunctions();
        $c = $ts->getClasses();
        $i = $ts->getInterfaces();
...
\class_alias('_PhpScoper5bf3cbdac76b4\\PHP_Token_FunctionTest', 'PHP_Token_FunctionTest', \false);
