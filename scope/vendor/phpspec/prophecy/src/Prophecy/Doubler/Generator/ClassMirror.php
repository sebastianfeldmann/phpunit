...
use _PhpScoper5bf3cbdac76b4\Prophecy\Exception\Doubler\ClassMirrorException;
use ReflectionClass;
...
class ClassMirror
{
    ...
    {
        $node = new \_PhpScoper5bf3cbdac76b4\Prophecy\Doubler\Generator\Node\ClassNode();
        if (null !== $class) {
            if (\true === $class->isInterface()) {
                throw new \_PhpScoper5bf3cbdac76b4\Prophecy\Exception\InvalidArgumentException(\sprintf("Could not reflect %s as a class, because it\n" . "is interface - use the second argument instead.", $class->getName()));
            }
        ...
        foreach ($interfaces as $interface) {
            ...
            if (\false === $interface->isInterface()) {
                throw new \_PhpScoper5bf3cbdac76b4\Prophecy\Exception\InvalidArgumentException(\sprintf("Could not reflect %s as an interface, because it\n" . "is class - use the first argument instead.", $interface->getName()));
            }
            $this->reflectInterfaceToNode($interface, $node);
        }
        $node->addInterface('_PhpScoper5bf3cbdac76b4\\Prophecy\\Doubler\\Generator\\ReflectionInterface');
        return $node;
    }
    private function reflectClassToNode(\ReflectionClass $class, \_PhpScoper5bf3cbdac76b4\Prophecy\Doubler\Generator\Node\ClassNode $node)
    {
        if (\true === $class->isFinal()) {
            throw new \_PhpScoper5bf3cbdac76b4\Prophecy\Exception\Doubler\ClassMirrorException(\sprintf('Could not reflect class %s as it is marked final.', $class->getName()), $class);
        }
    ...
    private function reflectInterfaceToNode(\ReflectionClass $interface, \_PhpScoper5bf3cbdac76b4\Prophecy\Doubler\Generator\Node\ClassNode $node)
    {
    ...
    private function reflectMethodToNode(\ReflectionMethod $method, \_PhpScoper5bf3cbdac76b4\Prophecy\Doubler\Generator\Node\ClassNode $classNode)
    {
        $node = new \_PhpScoper5bf3cbdac76b4\Prophecy\Doubler\Generator\Node\MethodNode($method->getName());
    ...
    private function reflectArgumentToNode(\ReflectionParameter $parameter, \_PhpScoper5bf3cbdac76b4\Prophecy\Doubler\Generator\Node\MethodNode $methodNode)
    {
        $name = $parameter->getName() == '...' ? '__dot_dot_dot__' : $parameter->getName();
        $node = new \_PhpScoper5bf3cbdac76b4\Prophecy\Doubler\Generator\Node\ArgumentNode($name);
