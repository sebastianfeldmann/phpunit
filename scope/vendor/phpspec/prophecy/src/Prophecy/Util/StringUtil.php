 ...
 */
namespace _PhpScoper5bf3cbdac76b4\Prophecy\Util;
...
class StringUtil
{
    ...
    public function stringify($value, $exportObject = \true)
    {
        ...
        if (\is_object($value)) {
            return $exportObject ? \_PhpScoper5bf3cbdac76b4\Prophecy\Util\ExportUtil::export($value) : \sprintf('%s:%s', \get_class($value), \spl_object_hash($value));
        }
    ...
    {
        $self = $this;
        return \implode(\PHP_EOL, \array_map(function (\_PhpScoper5bf3cbdac76b4\Prophecy\Call\Call $call) use($self) {
