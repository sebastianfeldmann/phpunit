 ...
 */
namespace _PhpScoper5bf3cbdac76b4\Prophecy\Call;
...
use _PhpScoper5bf3cbdac76b4\Prophecy\Exception\Call\UnexpectedCallException;
/**
...
class CallCenter
{
     ...
     */
    public function makeCall(\_PhpScoper5bf3cbdac76b4\Prophecy\Prophecy\ObjectProphecy $prophecy, $methodName, array $arguments)
    {
        ...
        if ('__destruct' === $methodName || 0 == \count($prophecy->getMethodProphecies())) {
            $this->recordedCalls[] = new \_PhpScoper5bf3cbdac76b4\Prophecy\Call\Call($methodName, $arguments, null, null, $file, $line);
            return null;
        }
        ...
        if ($methodProphecy->hasReturnVoid() && $returnValue !== null) {
            throw new \_PhpScoper5bf3cbdac76b4\Prophecy\Exception\Prophecy\MethodProphecyException("The method \"{$methodName}\" has a void return type, but the promise returned a value", $methodProphecy);
        }
        $this->recordedCalls[] = $call = new \_PhpScoper5bf3cbdac76b4\Prophecy\Call\Call($methodName, $arguments, $returnValue, $exception, $file, $line);
     ...
     * @return Call[]
     */
    public function findCalls($methodName, \_PhpScoper5bf3cbdac76b4\Prophecy\Argument\ArgumentsWildcard $wildcard)
    {
        return \array_values(\array_filter($this->recordedCalls, function (\_PhpScoper5bf3cbdac76b4\Prophecy\Call\Call $call) use($methodName, $wildcard) {
            return $methodName === $call->getMethodName() && 0 < $call->getScore($wildcard);
        }));
    }
    private function createUnexpectedCallException(\_PhpScoper5bf3cbdac76b4\Prophecy\Prophecy\ObjectProphecy $prophecy, $methodName, array $arguments)
    {
        ...
        return new \_PhpScoper5bf3cbdac76b4\Prophecy\Exception\Call\UnexpectedCallException(\sprintf("Unexpected method call on %s:\n" . "  - %s(\n" . "%s\n" . "    )\n" . "expected calls were:\n" . "%s", $classname, $methodName, $argstring, \implode("\n", $expected)), $prophecy, $methodName, $arguments);
    }
    private function formatExceptionMessage(\_PhpScoper5bf3cbdac76b4\Prophecy\Prophecy\MethodProphecy $methodProphecy)
    {
        return \sprintf("  - %s(\n" . "%s\n" . "    )", $methodProphecy->getMethodName(), \implode(",\n", $this->indentArguments(\array_map(function ($token) {
            return (string) $token;
        }, $methodProphecy->getArgumentsWildcard()->getTokens()), $indentationLength)));
    }
